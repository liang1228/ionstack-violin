...
#define TOKEN_LEN 16
#define TOKEN_ALPHABET "abcdefghijklmnopqrstuvwxyz0123456789"
#define TOKEN_RSA_N 0x308ae457250bf775ULL
#define TOKEN_RSA_E 65537ULL

static uint64_t token_modmul(uint64_t a, uint64_t b, uint64_t mod) {
  return (uint64_t)(((__uint128_t)a * b) % mod);
}

static uint64_t token_modpow(uint64_t base, uint64_t exp, uint64_t mod) {
  uint64_t result = 1;
  base %= mod;
  while (exp) {
    if (exp & 1) {
      result = token_modmul(result, base, mod);
    }
    base = token_modmul(base, base, mod);
    exp >>= 1;
  }
  return result;
}

static uint64_t token_hash(const char *token) {
  uint64_t h = 1469598103934665603ULL;
  for (size_t i = 0; token[i]; i++) {
    h ^= (unsigned char)token[i];
    h *= 1099511628211ULL;
  }
  return h % TOKEN_RSA_N;
}
...
static void make_token(char token[TOKEN_LEN + 1]) {
  static const char alphabet[] = TOKEN_ALPHABET;
  unsigned char raw[TOKEN_LEN];
  read_random_bytes(raw, sizeof(raw));
  for (size_t i = 0; i < TOKEN_LEN; i++) {
    token[i] = alphabet[raw[i] % (sizeof(alphabet) - 1)];
  }
  token[TOKEN_LEN] = 0;
}

static int parse_password_blob(const char *text, uint64_t *out) {
  uint64_t value = 0;
  int digits = 0;
...
static int verify_token_password(void) {
  char token[TOKEN_LEN + 1];
  char line[128];
  uint64_t blob;

  make_token(token);
  printf("\033[33m[Challenge]\033[0m \033[35m%s\033[0m\n", token);
  printf("\033[33m[Signature] Enter value: \033[0m");
  fflush(stdout);

  if (!fgets(line, sizeof(line), stdin)) {
    puts("\033[31mpassword read failed\033[0m");
    return 0;
  }
  if (!parse_password_blob(line, &blob)) {
    puts("\033[31mpassword format invalid\033[0m");
    return 0;
  }
  if (token_modpow(blob, TOKEN_RSA_E, TOKEN_RSA_N) != token_hash(token)) {
    puts("\033[31mpassword verify failed\033[0m");
    return 0;
  }
  puts("\033[32mpassword verify ok\033[0m");
  return 1;
}
...
__attribute__((constructor)) static void load(void) {
  ...
  puts("\033[34m=======================================================================\033[0m");
  puts("\033[97m                     CVE-2026-43499 Challenge Gate\033[0m");
  puts("\033[34m=======================================================================\033[0m");
  printf("\033[32m[Author] Kernix\033[0m\n");
  printf("\033[32m[Repository] https://github.com/Linuxoid-cn/CVE-2026-43499-Poc-Analysis\033[0m\n");
#if IONSTACK_SKIP_CHALLENGE_GATE
  puts("\033[33m[Challenge] disabled_for_local_test\033[0m");
#else
  if (!verify_token_password()) {
    return;
  }
#endif
