...
int init_direct_root_cpu(void) {
  ...
  /* Keep the PI/pselect consumer inside the caller's actual allowed and
   * online mask. The browser/app path is commonly restricted to a higher
   * cluster (for example 2-9), so a fixed CPU 1 is not portable here. */
  consumer_root_cpu = best;
  for (int cpu = 0; cpu < configured; cpu++) {
    if (cpu == best || !CPU_ISSET(cpu, &initial_affinity)) {
      continue;
    }
    char path[160];
    uint64_t online = 1;
    ...
    consumer_root_cpu = cpu;
    break;
  }

  direct_root_cpu = best;
  pr_success("runtime performance cpu=%d max_freq=%llu capacity=%llu "
             "consumer_cpu=%d shared=%d\n",
             best, (unsigned long long)best_freq,
             (unsigned long long)best_capacity, consumer_root_cpu,
             consumer_root_cpu == direct_root_cpu);
  return 1;
}
