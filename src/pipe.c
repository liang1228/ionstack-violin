...
void reset_pipe_attempt(void) {
  ...
  memset(physrw_after_write, 0, sizeof(physrw_after_write));
  physrw_read_ok = 0;
  physrw_write_ok = 0;
  physrw_read64_before = 0;
  physrw_read64_after = 0;
  physrw_write64_value = 0;
  physrw_read64_ok = 0;
  physrw_write64_ok = 0;
...
int install_pipe_physrw(int fd) {
  /* Never carry a proof bit from a previous cache attempt into Stage 2. */
  physrw_read_ok = 0;
  physrw_write_ok = 0;
  physrw_read64_ok = 0;
  physrw_write64_ok = 0;
