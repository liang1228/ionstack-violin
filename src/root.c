...
int root_child_done;
int root_stage_entered;
int root_stage_transport_ok;
int root_stage_ok;
...
int spawn_root_child(void) {
  ...
  /* Keep the parent-side descriptor state truthful.  Leaving the closed
   * number in the global pair lets a later abort path close an unrelated fd
   * if the kernel reuses that number during the root-stage probes. */
  root_ready_pipe[1] = -1;
...
static void abort_root_child(void) {
  if (root_child_pid > 0) {
    if (kill(root_child_pid, SIGKILL) != 0 && errno != ESRCH) {
      pr_info("root abort kill failed child=%d errno=%d\n",
              root_child_pid, errno);
    }
    while (waitpid(root_child_pid, NULL, 0) < 0 && errno == EINTR) {
    }
    root_child_pid = -1;
  }
  if (root_ready_pipe[0] >= 0) {
    close(root_ready_pipe[0]);
    root_ready_pipe[0] = -1;
  }
  if (root_ready_pipe[1] >= 0) {
    close(root_ready_pipe[1]);
    root_ready_pipe[1] = -1;
  }
  if (root_shared) {
    munmap(root_shared, sizeof(*root_shared));
    root_shared = NULL;
  }
}

/* Stage 2 is intentionally a separate gate from the first-stage SELinux
 * state.  A permissive byte alone is not root: the pipe transport must first
 * be proven and install_android_root() must verify the child credentials and
 * SELinux readback. */
int root_stage(int fd) {
  root_stage_entered = 1;
  root_stage_transport_ok =
    pipe_cache_gate_ok && physrw_read_ok && physrw_write_ok &&
    physrw_read64_ok && physrw_write64_ok;
  root_stage_ok = 0;
  pr_info("ROOT_STAGE_ENTER fd=%d transport=%d proof=%d/%d rw64=%d/%d\n",
          fd, root_stage_transport_ok, physrw_read_ok, physrw_write_ok,
          physrw_read64_ok, physrw_write64_ok);
  if (!root_stage_transport_ok) {
    pr_info("ROOT_STAGE_GATE_FAIL cache=%d read=%d write=%d read64=%d write64=%d\n",
            pipe_cache_gate_ok, physrw_read_ok, physrw_write_ok,
            physrw_read64_ok, physrw_write64_ok);
    return 0;
  }

  root_stage_ok = install_android_root(fd);
  if (!root_stage_ok) {
    abort_root_child();
  }
  pr_info("ROOT_STAGE_RESULT ok=%d child_done=%d uid=%u->%u selinux=%u->%u\n",
          root_stage_ok, root_child_done, root_uid_before, root_uid_after,
          selinux_before, selinux_after);
  return root_stage_ok;
}
